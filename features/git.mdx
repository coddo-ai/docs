---
title: "Git Integration"
description: "Manage branches, commits, merges, and worktrees — all from the Coddo UI."
icon: "code-branch"
---

Coddo includes a full **Git integration** so you never have to leave the app to manage version control. The Git view has three tabs: Branches, Changes, and History.

## Branches tab

View and manage all your local branches:

- **Create** new branches with sanitized names
- **Switch** between branches
- **Delete** branches you no longer need
- See the **current branch** highlighted
- View **ahead/behind** status compared to the remote

## Changes tab

A visual staging area for your Git workflow:

<Steps>
  <Step title="Review changes">
    See all modified, added, and deleted files with their status indicators.
  </Step>
  <Step title="Stage files">
    Stage individual files or all changes at once. Click on a file to see its diff.
  </Step>
  <Step title="Commit">
    Write a commit message and commit your staged changes.
  </Step>
  <Step title="Push">
    Push your commits to the remote repository.
  </Step>
</Steps>

## History tab

Browse the commit history of your current branch:

- View commit messages, authors, and timestamps
- See commit hashes
- Navigate through your project's history

## Worktrees

Coddo uses **Git worktrees** for task isolation. When a task runs in worktree mode:

1. A new worktree is created in a separate directory
2. A dedicated branch is checked out
3. Claude Code works in the isolated environment
4. After completion, changes can be merged back

This lets you run **multiple tasks in parallel** without code conflicts.

## Merge handling

When merging branches (e.g., after a task completes), Coddo handles:

- **Clean merges** — Automatic merge with no conflicts
- **Merge conflicts** — Visual conflict detection with a list of conflicted files
- **Abort merge** — Cancel a merge in progress
- **Finalize merge** — Complete the merge after resolving conflicts

## Remote operations

Coddo supports standard remote Git operations:

| Operation | Description |
|-----------|-------------|
| **Fetch** | Download remote changes without merging |
| **Pull** | Fetch and merge remote changes |
| **Push** | Upload local commits to the remote |

## Pull requests

Create pull requests directly from Coddo when a task is done. The PR is linked to the task's branch and can be reviewed on GitHub.
